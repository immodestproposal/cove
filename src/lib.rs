...
//! primitives. Many of these traits parallel existing mechanisms such as [`From`]/[`Into`] or
//! [`TryFrom`]/[`TryInto`], but offer differing semantics tailored to numerical types.
//!
//! # Motivation
...
//!
//! To see where the gaps are with the existing mechanisms, consider these definitions:
//!
//! * `lossless cast`: a numerical cast where the casted value does not change when its type changes
//! * `lossy cast`: a numerical cast where the casted value changes when its type changes
//!
...
//! `lossy: no detection`.
//! * The cast is not provably lossless and it is important to detect whether it is and, if so,
//! to obtain the details of the lossiness (such as the lossy value itself). Let's call this `lossy:
//! detection, details`.
//! * The cast is not provably lossless and it is important to detect whether it is; however, the
//! details of the lossiness (such as the lossy value itself) are unimportant. We can further
//! subdivide this into whether floating point casts are supported, as that is relevant to a
//! discussion of standard mechanisms. Let's call these `lossy: detection, no details, fp` and
//! `lossy: detection, no details, no fp`.
//!
...
//! | lossy: detection, no details, no fp   | [`TryFrom`]/[`TryInto`]   |
//! | lossy: detection, no details, fp      | ???                       |
//!
...
//! search space when hunting for invalid numerical casts.
...
//! | lossy: detection, no details, no fp   | [`Cast`] or [`TryFrom`]/[`TryInto`]   |
//! | lossy: detection, no details, fp      | [`Cast`]                              |

// ///////// TODO: write about support for floating point, maybe re-merge the fp/no-fp lines and
// add a new ## section. Also general organization of the docs. Make the impls public for future
// extension. Split into impl and interface modules. Add support for NonZero* where applicable.
