//! Provides cove's primary casting traits
//!
//! # Usage
//! Cove provides a [`prelude`](crate::prelude) module to assist with importing the requisite 
//! extension traits. Most applications of cove will not require `use`ing anything more.
//!
//! ```
//! use cove::prelude::*;
//! ```
...
//! from calling [`Cast::cast`] and well as for its contained error types. A common cove usage, 
...
//! ```
//!
//! An overview of the available follow-on extension traits is provided here; see the documentation
//! for each trait for more in-depth details and semantics:
//! * [`Lossless`]: for compile-time lossless casts based on types alone (e.g. widening conversions)
...
//! * [`Lossy`]: for casts where lossiness is acceptable with no general guarantees on the accuracy
//!     * Most akin to the `as` keyword but self-documents the intent and works in generic contexts
//!     * Very situational: consider one of the other extension traits instead or check the 
//!         [`Result`] returned by [`Cast`] directly
//!     * Zero-overhead: generally optimizes to the same assembly as the `as` keyword
//! * [`AssumedLossless`]: for casts asserted to be lossless at runtime
...
//!     * Zero-overhead: generally optimizes to the same assembly as the `as` keyword
//! * [`Closest`]: for casts which can be lossy provided they get as close as the types allow
...
//! * [`Bitwise`]: for casts which preserve the bits rather than numerical value
...
//!
//! * [`Cast`] and [`Closest`] are supported for all casts between all primitive numerical types as 
//!     well as the `NonZero*` family of non-zero integers from [`core::num`].
//! * [`Lossy`] and [`AssumedLossless`] are supported whenever the target type is a primitive 
//!     number.
//! * [`Lossless`] is supported whenever [`From`]/[`Into`] is supported as well as to/from [`usize`] 
//!     / [`isize`] / [`NonZeroUsize`](core::num::NonZeroUsize) /
//!     [`NonZeroIsize`](core::num::NonZeroIsize) when this is guaranteed lossless on the target
//!     platform.
//! * [`Bitwise`] is supported whenever the source and target types are the same size and [`Lossy`] 
//!     or [`Lossless`] is supported.
//!
//! # Guidelines
//! It might seem challenging to determine which type of cast to use in which circumstances. While
//! one size rarely fits all in software, here are some quick guidelines which might be useful:
//!
//! * If [`From`]/[`Into`] are provided for your use case, use those instead of any of cove's casts
//! * Otherwise, for interfaces to be consumed by a third party:
...
//!     * If using cove's casts in the interface regardless, consider one of cove's 
//!         [`bounding traits`](crate::bounds)
//! * Otherwise, for implementations or internal interfaces:
//!     * Favor cove's casts over [`TryFrom`]/[`TryInto`] or the `as` keyword
//!     * Favor [`Lossless`] if provided for your use case and you'd rather detect portability 
//!         errors at compile time than runtime
//!     * Favor [`AssumedLossless`] if confident the cast will always be lossless but [`Lossless`] 
//!         isn't defined for the types
//!     * Favor [`Cast`] with error handling if only lossless casts should proceed and runtime 
//!         detection is required
//!     * Favor [`Closest`] when best-effort lossiness is acceptable
//!     * Favor [`Bitwise`] when only the bit values are relevant; this sees particular usage in FFI
//!     * Use [`Lossy`] in niche circumstances; favor this over the `as` keyword
//!         * Exception: in some const contexts it may be necessary to use the `as` keyword, since
//!             const trait support is limited
...
pub trait Cast {
    ...
    /// * [`Bitwise`]: for when the bits should be preserved rather than the numerical value
...
/// [`AssumedLossless`] potentially offers a minor performance improvement in release builds
/// * Compared to [`Result::unwrap_unchecked`] after using [`Cast`] / [`TryFrom`] / [`TryInto`],
...
/// Follow-on extension trait for casting the bits of the source type to the same bits as the 
/// target type
/// 
/// As a follow-on extension trait, this is intended to be applied to a [`Result`] returned from
/// [`Cast::cast`]. This trait supports those use cases where the bits of input type should be 
/// preserved rather than the numerical value. This is particularly useful in FFI; for example, 
/// many Win32 APIs will take e.g. a `u32` flags argument, but the constants to build those flags are
/// defined as `i32`s.
/// 
/// Note that bitwise casting between primitive integers is akin to using `as` or a [`Lossy`] 
/// extension trait, but can better convey intent if the goal is truly to preserve bits. Bitwise 
/// casting involving floating pointer numbers is very different from `as` or [`Lossy`].
/// 
/// Favor bitwise over [`core::mem::transmute`] for safety reasons.
///
/// # Support
/// Cove provides support for [`crate::casts::Bitwise`] between all primitive numbers of the same 
/// size, between all `NonZero*` integers of the same size, and from `NonZero*` to 
/// primitive numerical types of the same size (but not the reverse).
pub trait Bitwise<T> {
    /// Called on a [`Result`] returned from [`Cast::cast`] to use the bit-equivalent value of the 
    /// destination type.
    ///
    /// # Performance
    /// In an optimized build, the combination of [`Cast::cast`] and [`Bitwise::bitwise`] generally
    /// compiles to the same assembly as `core::mem::transmute` and thus is zero-overhead.
    ///
    /// # Examples
    /// ```
    /// use cove::prelude::*;
    ...
    ///
    /// // A simple bitwise cast that is the same as using `as` except with clearer intent
    /// assert_eq!(10u16.cast::<i16>().bitwise(), 10i16);
    /// 
    /// // A bitwise cast from floating point to integer: not at all like `as`
    /// assert_eq!((-36.015_754_699_707_031_25f32).cast::<u32>().bitwise(), 3_255_832_610u32);
    /// 
    /// // Numbers from the `NonZero*` family are also supported
    /// assert_eq!(NonZeroU64::new(98765).unwrap().cast::<i64>().bitwise(), 98765i64);
    /// ```
    /// 
    /// ```compile_fail
    /// use cove::prelude::*;
    /// 
    /// // Attempt to bitwise cast from i16 to i32; will fail to compile since the sizes differ
    /// let _ = -8i16.cast::<i32>().bitwise();
    /// ```
    fn bitwise(self) -> T;
}
...
///     point type, infinity is preserved across the cast
...
///     (regardless of whether the source is also floating point or an integer), the target's 
///     `MAX`/`MIN` is produced rather than any infinity value.
///
...
///     the `NonZero*` family of numbers); this parallels the behavior of `as`.
///
/// # Support
/// Cove provides support for [`Closest`] for all casts supported by [`Cast::cast`]; that is,
/// between all primitive numerical types and the members of the `NonZero*` family defined in 
/// [`core::num`].
pub trait Closest<T> {
...
/// [`NonZeroIsize`](core::num::NonZeroIsize) on a per-platform basis, while the latter does not.
///
...
/// addition, it supports casts involving [`usize`] / [`isize`] whenever they are also supported for
...
/// / [`i64`] respectively.
///
/// Furthermore, [`Lossless`] is supported for casts from the `NonZero*` family in [`core::num`]
...
/// [`NonZeroUsize`](core::num::NonZeroUsize) or to [`usize`] on 32-bit or wider platforms
...
/// since the origin value could be zero. It does support the reverse: `NonZero*` to primitive 
/// numbers.
/// 
/// # Safety
/// Lossless should only be implemented for casts which are truly lossless or undefined behavior
/// may result. Lossless is generally implemented on types used as error markers for lossless 
/// casts (e.g. [`LosslessCastError`](crate::errors::LosslessCastError)); as a rule of thumb, 
/// such a type should not be constructible.
/// 
...
pub unsafe trait Lossless<T> {
    /// Unwraps a [`Result`] returned from [`Cast::cast`], extracting its [`Ok`] variant without 
    /// runtime overhead or any possibility of panic. Will only compile for casts for which this 
    /// guarantee can be made on the target platform. May not be portable to other target platforms; 
    /// portability failures will be compilation errors rather than runtime bugs. 
    ///
    /// # Performance
    ...
    /// use cove::prelude::*;
    ///
    /// // Try to cast a float to an integer losslessly; will fail to compile on all platforms 
    /// // since this cannot be guaranteed to be lossless at compile time
...
/// oversight. In addition, this trait allows for use in [`generic contexts`](crate::bounds), and 
/// enables implementation of lossy casts on non-primitive types where applicable. Finally, it is 
/// more easily-searchable in a codebase than the `as` keyword, which is overloaded for 
/// non-numerical casts.
///
/// [`Lossy`] is rarely the correct cast for a given situation. In most use cases it is better
/// to use [`From`]/[`Into`], [`Cast`], or one of the other follow-on extension traits provided
/// by cove. That said, [`Lossy`] should usually be preferred over the raw `as` keyword; see the
/// [`motivation`](crate::docs::motivation) section for more discussion on this topic.
///
/// # Support
